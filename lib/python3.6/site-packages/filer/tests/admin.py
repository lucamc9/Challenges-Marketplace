...
class FilerFolderAdminUrlsTests(TestCase):
    ...
    def test_change_folder_owner_keep_name(self):
        ...
        another_admin = User.objects.create_admin(
...
class BulkOperationsMixin(object):
    ...
    def create_image(self, folder, filename=None):
        ...
        image_obj = Image.objects.create(owner=self.admin, original_filename=self.image_name, file=file_obj, folder=folder)
        image_obj.save()
        return image_obj
    ...
    def create_file(self, folder, filename=None):
        ...
        file_obj = File.objects.create(owner=self.admin, original_filename=filename, file=file_data, folder=folder)
        file_obj.save()
        return file_obj
...
class FilerBulkOperationsTests(BulkOperationsMixin, TestCase):
    ...
    #     self.assertEqual(self.src_folder.files.count(), 0)
    #     self.assertEqual(self.dst_folder.files.count(), 0)
    #     clipboard = Clipboard.objects.get(user=self.admin)
...
class FolderListingTest(TestCase):
    ...
    def setUp(self):
        ...
        self.parent = Folder.objects.create(name='bar', parent=None, owner=admin)

        self.foo_folder = Folder.objects.create(name='foo', parent=self.parent, owner=self.staff_user)
        self.bar_folder = Folder.objects.create(name='bar', parent=self.parent, owner=admin)
        self.baz_folder = Folder.objects.create(name='baz', parent=self.parent, owner=admin)

        file_data = django.core.files.base.ContentFile('some data')
        file_data.name = 'spam'
        self.spam_file = File.objects.create(
            owner=admin, original_filename='spam',
